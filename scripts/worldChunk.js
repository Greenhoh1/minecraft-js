...
export class WorldChunk extends THREE.Group {
  ...
  generateTrees(rng) {
    ...
    for (let baseX = canopySize; baseX < this.size.width - canopySize; baseX++) {
      for (let baseZ = canopySize; baseZ < this.size.width - canopySize; baseZ++) {
        ...
        // Store tree data in an array for later processing to minimize unnecessary checks
        treeData.push({ baseX, baseZ });
      }
    }

    // Process tree data to avoid repeating the same checks
    treeData.forEach(({ baseX, baseZ }) => {
      for (let y = this.size.height - 1; y--; y >= 0) {
        if (this.getBlock(baseX, y, baseZ).id !== blocks.grass.id) continue;

        const baseY = y + 1;
        const trunkHeight = Math.round(rng.random() * (this.params.trees.trunkHeight.max - this.params.trees.trunkHeight.min)) + this.params.trees.trunkHeight.min;
        ...
        const R = Math.round(rng.random() * (this.params.trees.canopy.size.max - this.params.trees.canopy.size.min)) + this.params.trees.canopy.size.min;
        for (let x = -R; x <= R; x++) {
          for (let y = -R; y <= R; y++) {
            for (let z = -R; z <= R; z++) {
  ...
  generateClouds(rng) {
    const simplex = new SimplexNoise(rng);
    const cloudMesh = new THREE.InstancedMesh(geometry, new THREE.MeshLambertMaterial({ color: 0x9090e0, transparent: true, opacity: 0.5 }), this.size.width * this.size.width);
    ...
    for (let x = 0; x < this.size.width; x++) {
      for (let z = 0; z < this.size.width; z++) {
        ...
        if (value < this.params.clouds.density) {
          const instanceId = cloudMesh.count++;
          matrix.setPosition(x, this.size.height - 1, z);
          cloudMesh.setMatrixAt(instanceId, matrix);
        }
      }
    }
