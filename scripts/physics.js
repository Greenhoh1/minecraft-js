...
export class Physics {
   ...
   * @param {Array<{x: number, y: number, z: number}>} candidates
   * @param {Player} player
   * @returns {Array<{ block: Object, contactPoint: Object, normal: THREE.Vector3, overlap: number }>}
   */
   ...
   * @param {Array<{ block: Object, contactPoint: Object, normal: THREE.Vector3, overlap: number }>} collisions
   * @param {Player} player
   */
  resolveCollisions(collisions, player) {
    ...
    for (const collision of collisions) {
      // Re-check if the contact point is inside the player bounding cylinder
  ...
  addContactPointerHelper(p) {
    ...
    contactMesh.position.set(p.x, p.y, p.z);
